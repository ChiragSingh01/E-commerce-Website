                ...
                <div class ="d-flex flex-column vh-100 justify-content">
                    <div class="d-flex flex-row">
                        <?php                     
                            ...
                            $sql_obj=mysqli_query($conn,"select * from products,address,invoice where address.userId=invoice.userId and invoice.vendorId=$_SESSION[userid] and invoice.proId=product_ID");
                                                
                            while($row=mysqli_fetch_assoc($sql_obj)){
                                $concatenatedString1 = $row['address'] . "," . $row['address2'] . "," . $row['district'].",".$row['city'].",".$row['postal_code'];        
                                echo "
                                    <div>
                                        <div class='pdt-card'>
                                            <a href='Product_over_view.php?pro_id=$row[product_ID]' style='text-decoration: none;'>
                                                ...
                                                <div class='price'><p><strong>Invoice ID:</strong> $row[invoiceId]</p></div>
                                                <div class='price'><p><strong>Billing Address:</strong> $concatenatedString1</p></div>
                                                <div class='price'><p><strong>Billing Date:</strong> $row[CreatedDate]</p></div>
                                            <a>  
